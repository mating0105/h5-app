            ...
            <Card class="xh-top-10">
              <template v-slot:header>
                <section class="xh-plus head">
                  <div>
                    <van-cell
                      :title="item.creditTypeDesc?item.creditTypeDesc:returnText(
                              dictionaryData,
                              'business_type',
                              item.businesstype
                            )"
              ...
              <van-row
                style="padding:10px 10px 20px;line-height:22px;"
                @click="applyPay(item)"
              >
                <van-col span="24"> 客户编号：{{ item.projectNum }} </van-col>
...
export default {
  ...
  data() {
    ...
    return {
      dayjs: dayjs,
      listData: [],
      tabList: [{ name: "待办", value: "1" }, { name: "已办", value: "3" }],
      active: "1", //待办
      loading: false,
      listLoading: false,
      isLoading: false,
      ...
      dealWithData: {
        ...
        activityId: [
          {
            label: "客户经理待办",
            value: "WF_PROJ_APPR_01_T01",
            processName: "审批流程"
          },
          {
            label: "内勤待办",
            value: "WF_PROJ_APPR_01_T04",
            processName: "审批流程"
          },
          {
            label: "审批官待办",
            value: "WF_PROJ_APPR_01_T52",
            processName: "审批流程"
          },
          {
            label: "业务人员待办",
            value: "WF_CAPTURE_001_T001",
            processName: "垫款流程"
          },
          {
            label: "财务垫款确认待办",
            value: "WF_CAPTURE_001_T002",
            processName: "垫款流程"
          },
          {
            label: "客户征信经理待办",
            value: "WF_CU_CREDIT_001_T001",
            processName: "征信流程"
          },
          {
            label: "驻行人员待办",
            value: "WF_CU_CREDIT_001_T003",
            processName: "征信流程"
          },
          {
            label: "合规文员待办",
            value: "WF_BANK_MAKE_LOAN_001_T002",
            processName: "放款流程"
          },
          {
            label: "驻行内勤待办",
            value: "WF_BANK_MAKE_LOAN_001_T004",
            processName: "放款流程"
          }
        ]
      },
      ...
      pathData: {
        ...
        dealwith: [
          ...
          { name: "驻行人员待办", path: "/bankCreditReply", params: newParams,creditType:'1' },//银行征信
          { name: "驻行人员待办", path: "/bigDataReply", params: newParams,creditType:'2' },//大数据征信
          { name: "驻行人员待办", path: "/rbReply", params: newParams,creditType:'3' },//人保征信
  ...
  methods: {
    ...
    applyPay(val) {
      ...
      if (this.pageData.status == 1) {
        //待办
        console.log(this.returnText(this.dealWithData, "activityId", val.activityId))
          if(this.returnText(this.dealWithData, "activityId", val.activityId)  == '驻行人员待办'){
            if(val.creditType){
              this.returnPath(val.creditType,"dealwith",'E');
            }else{
              this.returnPath(
                this.returnText(this.dealWithData, "activityId", val.activityId),
                "dealwith"
              );
            }
          }else{
    ...
    returnPath(value, n,label) {
      this.pathData[n].forEach((item, index) => {
        if(label){
          if (item.creditType == value) {
            this.$router.push({
              path: item.path,
